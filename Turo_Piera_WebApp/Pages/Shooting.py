...
def create_clearance_map(df, team_name, matchweek):
    ...
    team_data = df[(df['Team'] == team_name) & (df['Matchweek'] == matchweek) & (df['Event'] == 'Shot')]
    ...
    for i in range(len(team_data)):
        plt.plot(
            ...
            color='#131883', linewidth=2, alpha=0.7
        )
        plt.scatter(
            team_data.iloc[i]['X'], team_data.iloc[i]['Y'], 
            color='#131883', s=500, edgecolor='#ffffff', zorder=3  
        )
        plt.text(
            team_data.iloc[i]['X'], team_data.iloc[i]['Y'], 
    ...
    plt.title(f'{team_name} Shot Map for {matchweek}', color='#ffffff', size=25)
