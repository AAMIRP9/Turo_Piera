...
def create_tackle_map(df, team_name, matchweek):
    ...
    team_data = df[(df['Team'] == team_name) & (df['Matchweek'] == matchweek) & (df['Event'] == 'Save')]
    ...
    for i in range(len(team_data)):
        ...
        plt.scatter(
            ...
            color='#00ffdd', s=500, edgecolor='#000000', zorder=3  # Red-orange color for visibility with black edges
        )
        plt.text(
            team_data.iloc[i]['X'], team_data.iloc[i]['Y'], 
            str(team_data.iloc[i]['Player']), 
            color='#000000', fontsize=12, ha='center', va='center', zorder=4  # Black text inside the circle for high contrast
        )

    # Add title for the team's tackle map
    plt.title(f'{team_name} Tackle Map for {matchweek}', color='#ffffff', size=20)
